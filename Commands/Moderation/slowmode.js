...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption(options => options.setName("rate").setDescription("Provide the rate at which the user can send a new message, e.g 5s, 1m, 30m, etc.").setRequired(true))
        .addStringOption(options => options.setName("reason").setDescription("Provide a reason for why you activated slowmode.").setRequired(false))
        .addStringOption(options => options.setName("duration").setDescription("Provide a duration for the slow mode, e.g 5s, 1m, 30m, etc., after which it will disable itself.").setRequired(false)),
    async execute(interaction) {
        ...
        const description = duration ? `Slow mode has been enabled with a rate of ${ms(rate, { long: true, })} for ${ms(duration, { long: true })}` : `Slow mode has been enabled with a rate of ${ms(rate, { long: true })}`;
